...
class Login_process extends CI_Controller {
	...
	public function id_search() {
		...
		preg_match('/[a-zA-Z!#$%^&*()?+=\/]/', $this -> input -> post('form_number'), $user_number_check);
		preg_match('/[!#$%^&*()?+=\/]/', $this -> input -> post('form_email1'), $user_email1_check);
		preg_match('/[!#$%^&*()?+=\/]/', $this -> input -> post('form_email2'), $user_email2_check);
		
		if ($this -> input -> post('form_name') == null) {
			alert('이름을 입력하시지 않았습니다.');
		} else if ($this -> input -> post('form_number') == null) {
			alert('학번을 입력하시지 않았습니다.');
		} else if (strlen($this -> input -> post('form_number')) != 8) {
			alert('학번이 올바르지 않습니다.');
			...
			alert('올바른 e-mail형식이 아닙니다.');
		} else if (strlen($this -> input -> post('form_email2')) < 4) {
			alert('올바른 e-mail형식이 아닙니다.');
		}
	...
	public function pw_search() {
		...
		preg_match('/[!#$%^&*()?+=\/]/', $this -> input -> post('form_email2'), $user_email1_check);
		preg_match('/[!#$%^&*()?+=\/]/', $this -> input -> post('form_email2'), $user_email2_check);
		
		if ($this -> input -> post('form_id') == null) {
			alert('아이디를 입력하시지 않았습니다.');
		} else if ((strlen($this -> input -> post('form_id')) < 5) || (strlen($this -> input -> post('form_id')) > 12)) {
			alert('올바른 아이디가 아닙니다.');
